  ...
  <article class="border-bottom">
    ...
    >
      <div class="fr-col-auto fr-grid-row fr-grid-row--top no-wrap">
        <div
          ...
          v-html="resourceImage"
          aria-hidden="true"
        ></div>
        <div class="fr-col-auto">
          <h4 class="fr-mb-1v" :id="'resource-' + resource.id + '-title'">
          ...
          <div class="fr-text--sm fr-mb-0 text-grey-380">
            <p style="margin: 0">
              ...
              <template v-else-if="resource.organization">
                {{ $t("From") }}
                <a :href="resource.organization.page">{{ owner }}</a> —
              </template>
              {{
              ...
              <template v-if="resource.format">
                ...
                <template v-if="resource.filesize"
                  >({{ filters.filesize(resource.filesize) }})</template
                >
                —
              </template>
          ...
          <li class="fr-col-auto fr-mr-3v" v-if="resource.preview_url">
            <button
              ...
              @click.prevent="
                showModal(
                  'preview',
                  { url: resource.preview_url, title: resource.title },
                  true
                )
              "
    ...
    >
      <div
        ...
        v-html="filters.markdown(resource.description)"
      ></div>
      <dl>
        ...
        <div class="fr-grid-row fr-grid-row--gutters fr-mb-2w">
          <dt class="fr-col-4 fr-col-md-3 fr-col-lg-2">
            {{ $t("Permalink") }}
          </dt>
          <dd class="fr-ml-0 fr-col-8 fr-col-md-9 fr-col-lg-10">
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const availabilityChecked = computed(
      () => props.resource.extras && props.resource.extras["check:status"]
    );
    const lastUpdate = computed(() =>
      props.resource.published > props.resource.last_modified
        ? props.resource.published
        : props.resource.last_modified
    );
    const unavailable = computed(
      () => availabilityChecked.value && availabilityChecked.value >= 400
    );
    const showSchemaButton = computed(
      () => props.resource.schema && props.resource.schema.name
    );
    return {
      owner,
      resourceImage,
